 ...
 */
module.exports = (function (_$, NS, decode_next_handler) {
  ...
  const doNextAsync = (that, callback) => Promise.resolve(that)
    .then((that) => {
      //! decode parameter.
      const ID = that._id;
  ...
  const main = function (event, context, callback) {
    ...
    const that = {
      _id: ID, _mode: MODE, _cmd: CMD, _param: $param, _body: $body, _event: event, _ctx: context,
    };
