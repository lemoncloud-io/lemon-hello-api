...
module.exports = (function factory($scope, $environ) {
  ...
  const $console = {
    thiz: console, log: console.log, error: console.error, auto_ts: TS, auto_color: LC,
  };
  const _log = function () {
  ...
  const _extend = function (opt, opts) { // simple object extender.
    for (const k in opts) {
      const v = opts[k];
  ...
  function _ts(_d) { // timestamp like 2016-12-08 13:30:44
    ...
    const [y, m, d, h, i, s] = [dt.getFullYear(), dt.getMonth() + 1, dt.getDate(), dt.getHours(), dt.getMinutes(), dt.getSeconds()];
